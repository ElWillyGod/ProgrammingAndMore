...
Esta es una forma de menar un grupo de gorutinas, su función generalmente es permitir que una gorutina espere a otro conjunto de gorutinas terminen su ejecución, eso es para asegurar que el programa principal espere a que todas las gorutinas secundarias finalicen antes de continuar. Funciona mediante un contador interno que es incrementado con `Add(delta int)` (generalmente 1) y se decrementa con `Done()` que es llamada desde dentro de la función para indicar que ya termino la gorutina interna, finalmente se usa `Wait()` esto bloquea la ejecución de la gorutina hasta que el contador interno llegue a cero.